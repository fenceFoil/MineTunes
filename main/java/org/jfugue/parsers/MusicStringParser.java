...
public final class MusicStringParser extends Parser {
	...
	/**
	 * Returns true if a default tempo event (120bpm) is generated when a
	 * pattern doesn't specify an initial tempo.
	 * 
	 * @return true if a default tempo event should be generated when a pattern
	 *         is parsed.
	 */
	...
	/**
	 * Set whether a default tempo event (120bpm) should be generated at the
	 * beginning of any pattern that doesn't specify an initial tempo.
	 * 
	 * <p>
	 * When this value is <code>true</code> any pattern that doesn't start with
	 * a <code>Tempo</code> command will have an implicit <code>T120</code>
	 * command inserted.
	 * 
	 * @param defaultTempoEnabled
	 *            true if a default tempo event should be generated when a
	 *            pattern with no tempo is parsed.
	 */
	...
	private boolean requiresDefaultTempo(String[] tokl) {
		return isDefaultTempoEnabled() && tokl.length > 0
				&& tokl[0].toUpperCase().charAt(0) != 'T';
	}
	...
	private void parseToken(String s) throws JFugueException {
		...
		case '$':
			...
			break; // New in 2.0
		case '^':
			parseSystemExclusiveElement(s);
			break; // New in 4.1
		case '(':
			parseCollectedNoteElement(s);
			break; // New in 4.1
		case 'A':
		case 'B':
		case 'C':
	...
	/**
	 * MCDITTY: Change: unrecognized tokens throw an exception
	 * 
	 * This method takes a single token, and distributes it to a specific
	 * element parser based on the first character in the string. If the parser
	 * does not recognize the first character of the string, the token will be
	 ...
	 *             if there is a problem parsing the string
	 */
	public void parseTokenStrict(String s) throws JFugueException {
		// If there are any spaces, get out
		if (s.indexOf(" ") != -1) {
			throw new ParserError(ParserError.PARSER_SPACES_EXC, s, s);
		}
		...
		case '~':
			...
			break; // MCDitty
		default:
			new JFugueException("Not a MusicString token: ", s);
		}
	}

	/**
	 * MCDitty: Parses a Lyric element.
	 * 
	 * @param s
	 */
	private void parseLyricElement(String s) {
		if (s.length() > 1) {
			...
			throw new JFugueException(JFugueException.EMPTY_LYRIC_EXCEPTION, s);
		}
	}
	...
	private void parseTempoElement(String s) throws JFugueException {
		...
		// MCDITTY: Range check
		int tempoForRangeCheck = 50; // ANY PASSING VALUE
		if (s.substring(1, s.length()).matches("\\d*")) {
			try {
				tempoForRangeCheck = Integer
						.parseInt(s.substring(1, s.length()));
			} catch (NumberFormatException e) {
				e.printStackTrace();
			}
		}
		if (tempoForRangeCheck < MIN_TEMPO) {
			throw new JFugueException(
					JFugueException.TEMPO_LOW_RANGE_EXCEPTION,
					tempoForRangeCheck);
		} else if (tempoForRangeCheck > MAX_TEMPO) {
			throw new JFugueException(
					JFugueException.TEMPO_HIGH_RANGE_EXCEPTION,
					tempoForRangeCheck);
		}
	...
	private long getLongValueOfToken(String token) throws JFugueException {
	...
	private void parseSystemExclusiveElement(String s) throws JFugueException {
		...
		byte[] data = parseSystemExclusiveData(
				s.substring(indexOfColon + 1, s.length()), radix);
	 ...
	 * 
	 * @param keysigToken
	 *            a token representing a MusicString key signature
	 * @returns 0 for a major key; 1 for a minor key.
	 */
	private int getMIDIKeySignatureScale(String keysigToken)
			throws JFugueException {
	...
	private int scaleAbbreviationToMIDIScale(String scale)
			throws JFugueException {
	...
	private void parsePolyPressureElement(String s) throws JFugueException {
		...
		trace("PolyphonicPressure element: key = " + keyNumber
				+ ", pressure = " + pressureNumber);
		firePolyphonicPressureEvent(new PolyphonicPressure(keyNumber,
				pressureNumber));
	}
	...
	private byte getPolyPressurePressure(String polyPressureToken) {
		return getByteFromDictionary(polyPressureToken
				.substring(polyPressureToken.indexOf(',') + 1));
	}
	...
	private void parsePitchBendElement(String s) throws JFugueException {
		...
		trace("PitchBend element: byte1 = " + bytes[0] + ", byte2 = "
				+ bytes[1]);
	...
	/**
	 * Given a String representation of an int, breaks the int that it
	 * represents into two bytes.
	 * 
	 * @returns An array of two bytes, [0] is the Least Significant Byte, [1]
	 ...
	 * same duration, attack, and decay. For example, (C+E+G)q This method works
	 * by parsing the notes and building a new Music String that will then be
	 * sent to parseNoteElement. For example, (C+E+G)q will be converted into
	 * Cq+Eq+Gq, which will then be parsed using existing listeners.
	 * 
	 * Underscore characters, which are used to indicate notes that play
	 * alongside other notes, are not allowed because all notes in a collected
	 * note element will have the same duration.
	 * 
	 * @param s
	 *            the token that contains the collected note element
	 * @throws JFugueException
	 *             if there is a problem parsing the element
	 */
	private void parseCollectedNoteElement(String s) throws ParserError {
		if (s.indexOf("_") > -1) {
			throw new ParserError(
					"The character '_' is not a valid character in this collected note element: "
							+ s);
		}
	...
	private int parseNoteRoot(String s, int slen, int index, NoteContext context) {
		...
		case '[':
			return parseBracketedNote(s, slen, index, context);
		case 'R':
	...
	private int parseBracketedNote(String s, int slen, int index,
			NoteContext context) {
	...
	private int parseLetterNote(String s, int slen, int index,
		...
		case 'B':
			...
			break;
		default:
			throw new ParserError(ParserError.NOTE_EXC, s);
		}
		index++;
	...
	private int parseNoteOctave(String s, int slen, int index,
		...
		if ((possibleOctave1 >= '0') && (possibleOctave1 <= '9')) {
			...
			String octaveNumberString = s.substring(index, index
					+ definiteOctaveLength);
			trace("Octave spec is " + octaveNumberString);
			try {
				context.octaveNumber = Byte.parseByte(octaveNumberString);
			} catch (NumberFormatException e) {
				throw new ParserError(ParserError.OCTAVE_EXC,
	...
	private int parseNoteChord(String s, int slen, int index,
		...
		if (lengthOfChordString > 0) {
			...
			trace("Chord: chordLength=" + lengthOfChordString
					...
					+ possibleChord3 + " 4=" + possibleChord4 + " 5="
					+ possibleChord5 + " 6=" + possibleChord6 + " 7="
					+ possibleChord7 + " 8=" + possibleChord8 + " ]");
		}
	...
	/**
	 * This is apparently the default {@link javax.sound.midi.Sequence}
	 * resolution.
	 * 
	 * @deprecated
	...
	private int parseNoteDuration(String s, int slen, int index,
		...
		// context.duration = (long) (128.0 * 4.0 * context.decimalDuration); //
		// javax.sound.midi.Sequence resolution is 120
		// 12/22/08 - As identified by E. Gingras, resolution should be 128, not
		// 120, for better compatability with 128th notes (and other durations)
		context.duration = (long) (JFugueDefinitions.SEQUENCE_RESOLUTION * context.decimalDuration); // DMK
																										// 9/27/08:
																										// The
																										// *4.0
																										// makes
	...
	private int parseNumericDuration(String s, int slen, int index,
		...
		if ('[' == s.charAt(index)) {
			...
			context.decimalDuration += getDoubleFromDictionary(s.substring(
					index + 1, indexOfEndingBracket));
	...
	private boolean isDecimalCharacter(char candidateCharacter) {
		return candidateCharacter == '.'
				|| ((candidateCharacter >= '0') && (candidateCharacter <= '9'));
	}
	...
	private int parseNoteVelocity(String s, int slen, int index,
		...
		while (index < slen) {
			...
			byte velocityNumber = getByteFromDictionary(s.substring(startPoint,
					endPoint));
	...
	private void fireNoteEvents(NoteContext context) {
		...
		if (context.isChord) {
			note.setChord(context.chordName,
					Arrays.copyOf(context.halfsteps, context.numHalfsteps));
			for (int i = 0; i < context.numHalfsteps; i++) {
	...
	/**
	 * MCDitty: parse MCDitty tokens that start with a ~
	 * 
	 * @param s
	 */
	private void parseMCDittyElement(String s) {
		fireMCDittyEvent(new MCDittyEvent(s));
	}
}
