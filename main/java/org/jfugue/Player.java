...
public class Player {
	...
	// MCDitty
	private DittyPlayerPlayingHookListener hookListener;
	private long runningTickTimeMS = 20; // The hard-coded default
	...
	// TODO: Get a dittyproperties somehow

	/**
	 * Add the softClose parameter to the constructor. This option changes how
	 * sequences are processed. If true sequences are not closed and then
	 * reopened, they are stopped and repositioned to the beginning. These
	 * solves the problem of sound stopping after 17 iterations.
	 * 
	 * @param connected
	 * @param softClose
	 */
	...
	/**
	 * MCDITTY NOTE: Only one hook listener is set at a time; this is more like
	 * "setHookListner".
	 */
	public void addHookListener(DittyPlayerPlayingHookListener l) {
		hookListener = l;
	}
	...
	private void initSequencer() {
		...
		ControllerEventListener cel = new ControllerEventListener() {
			public void controlChange(javax.sound.midi.ShortMessage event) {
				// System.out.println("got controllerEvent " +
				// event.toString());
			}
		};
	...
	/**
	 * Returns the instance of the MusicStringParser that the Player uses to
	 * parse music strings. You can attach additional ParserListeners to this
	 * Parser to know exactly when musical events are taking place. (Similarly,
	 * you could create an Anticipator with a delay of 0, but this is much more
	 * direct).
	 * 
	...
	/**
	 * Allows you to set the MusicStringParser for this Player. You shouldn't
	 * use this unless you definitely want to hijack the Player's
	 * MusicStringParser. You might decide to do this if you're creating a new
	 * subclass of MusicStringParser to test out some new functionality. Or, you
	 * might have set up a MusicStringParser with an assortment of
	 * ParserListeners and you want to use those listeners instead of Player's
	 * default listener (although really, you should call getParser() and add
	 * your listeners to that parser).
	 * 
	 * @param parser
	 *            Your new instance of a MusicStringParser
	 */
	 ...
	 * 
	 * @param context
	 *            A map of pattern identifiers to Pattern objects
	 * @param pattern
	 *            The pattern to play
	 */
	public void play(Map<String, Pattern> context, PatternInterface pattern) {
		PatternInterface contextPattern = Pattern.createPattern(context,
				pattern);
		play(contextPattern);
	}
	...
	/**
	 * Plays a {@link Rhythm} by setting up a Renderer and feeding the
	 * {@link Rhythm} to it.
	 * 
	 * @param rhythm
	 *            the {@link Rhythm} to play
	 * @see MidiRenderer
	 */
	...
	public void play(Sequence sequence) {
		...
		// MCDITTY: Activate player init hook
		if (hookListener != null) {
			hookListener.initPlayerHook();
		}
		...
		if (softClose) {
			...
			// MCDITTY: if this player was given a dittyProperties, run anything
			// that needs running right now
			while (isOn()) {
				try {
					// Fire hook event
					if (hookListener != null) {
						// hookListener.playerHook(getSequencer().getMicrosecondPosition());
						hookListener.playerHook(getSequencer()
								.getTickPosition(), getSequencer()
								.getTempoInBPM());
					}

					Thread.sleep(runningTickTimeMS); // don't hog all of the CPU
				} catch (InterruptedException e) {
	 ...
	 */
	public void playMidiDirectly(File file) throws IOException,
	 ...
	 * @throws InvalidMidiDataException
	 */
	public void playMidiDirectly(URL url) throws IOException,
			InvalidMidiDataException {
		Sequence sequence = MidiSystem.getSequence(url);
		play(sequence);
	}

	public void play(Anticipator anticipator, PatternInterface pattern,
			long offset) {
		Sequence sequence = getSequence(pattern);
	...
	/**
	 * Convenience method for playing multiple patterns simultaneously. Assumes
	 * that the patterns do not contain Voice tokens (such as "V0"). Assigns
	 * each pattern a voice, from 0 through 15, except 9 (which is the
	 * percussion track). For this reason, if more than 15 patterns are passed
	 * in, an IllegalArgumentException is thrown.
	 * 
	 * @param patterns
	 *            The patterns to play in harmony
	...
	public PatternInterface playInHarmony(Pattern... patterns) {
		if (patterns.length > 15) {
			throw new IllegalArgumentException(
					"playInHarmony no more than 15 patterns; "
							+ patterns.length + " were passed in");
		}
	 ...
	 * MCDitty
	 * 
	 * @return the runningTickTimeMS
	 */
	public long getRunningTickTimeMS() {
		return runningTickTimeMS;
	}

	/**
	 * MCDitty This sets the MS delay between calls to
	 * JFuguePlayerPlayingHook.hook... methods that are called as a sequence is
	 * playing.
	 * 
	 * @param runningTickTimeMS
	 *            the runningTickTimeMS to set
	 */
	public void setRunningTickTimeMS(long runningTickTimeMS) {
		this.runningTickTimeMS = runningTickTimeMS;
	}
	 ...
	 */
	public void saveMidi(PatternInterface pattern, File file)
			throws IOException {
		Sequence sequence = getSequence(pattern);
	...
	/**
	 * Parses a MIDI file and returns a Pattern representing the MIDI file. This
	 * is an excellent example of JFugue's Parser-Renderer architecture:
	 * 
	 * <pre>
	 ...
	 */
	public PatternInterface loadMidi(File file) throws IOException,
			InvalidMidiDataException {
		...
		PatternInterface pattern = new Pattern(renderer.getPattern()
				.getMusicString());
	...
	/**
	 * Stops all notes from playing on all MIDI channels. Uses the synthesizer
	 * provided by MidiSystem.getSynthesizer().
	 */
